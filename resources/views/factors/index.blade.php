<x-app-layout>
    @section('headertitle')
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('messages.Factors', [], session()->get('applocale')) }}                       
        </h2>
    @endsection

    @section('maincontent')
    <!-- Create New Factor Button -->
    <div class="max-w-12xl mx-auto sm:px-8 mt-2 lg:px-10">
        <div class="container">
            <div class="bg-white p-6 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="row">
                    <div class="col-md-8">
                        <p class="c-grey h4">{{ __('messages.Factors', [], session()->get('applocale')) }}</p>
                    </div>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('messages.Examples', [], session()->get('applocale')) }}</th>
                        ...
                        @foreach($factors as $factor)
                            ...
                            <tr>
                                ...
                                <td>{{ \Illuminate\Support\Str::limit($factor->example, 300) }}</td>
                                <td class="text-end align-middle">
                                    <div class="d-flex justify-content-end gap-2 flex-nowrap">
                                       ...
                                       <button
                                        ...
                                        <!-- Delete Form -->
                                        <form action="{{ route('factors.destroy', $factor->id) }}" method="POST" id="deleteForm-{{ $factor->id }}">
                                            @csrf
                                            @method('DELETE')
    ...
    <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form id="editFactorForm" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModalLabel">{{ __('messages.Edit Factor', [], session()->get('applocale')) }}</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="editExamples" class="form-label">{{ __('messages.Examples', [], session()->get('applocale')) }}</label>
                            <textarea class="form-control" id="editExamples" name="examples" rows="10" required></textarea>
                        </div>
                    </div>
    ...
    <div class="modal fade" id="createModal" tabindex="-1" aria-labelledby="createModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form action="{{ route('factors.store') }}" method="POST">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="createModalLabel">{{ __('messages.Create new factor', [], session()->get('applocale')) }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
    ...
    <script>
        ...
        function openEditModal(button) {
            const factor = JSON.parse(button.getAttribute('data-factor'));
            const url = '/factors/'+factor.id+'/edit';
            document.getElementById('editFactorForm').action = url;
            document.getElementById('editName').value = factor.name;
            document.getElementById('editDescription').value = factor.description;
            document.getElementById('editExamples').value = factor.example;
