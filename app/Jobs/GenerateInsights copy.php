...
class GenerateInsights implements ShouldQueue
{
    ...
    {
        Log::info("🚀 Starting GenerateInsights job for the user {$this->userId}");
    ...
    try {
        ...
        $entries = Entry::where('user_id', $this->userId)->whereHas('dform', function ($query) {
            ...
            if ($entries->count() >= 6) {
               ...
               /**foreach ($charts as $key => $chartData) {
                    $chartType = $chartMapping[$key] ?? $key;
                    
                    // Create a new Request instance and replace its data with the chart data and type.
                    $requestData = \Illuminate\Http\Request::create('/', 'GET', [
                         'chart' => $chartData,
                         'type'  => $chartType,
                    ]);
                                    
                    $response = $controller->generateResponse($requestData);
                    $responseData = method_exists($response, 'getData') ? (array)$response->getData(true) : null;
                    $insightSummaries[$key] = $responseData['response'] ?? 'failed';
                }*/
